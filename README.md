...
# Hostname used within the Heat Docker container.
...
If you require setting additional heat.conf configuration flags, please fork the docker-heat project, make your additions, test and submit a pull request to get your changes back upstream.
...
```
Now you can run a Heat container from the newly created image. You can use the run script or run the container manually.
...
```
**Option 2-** Manually:
The example below uses the -h flag to configure the hostame as heat.example.com within the container, exposes TCP ports 8000 and 8004 on the Docker host, names the container heat, uses -d to run the container as a daemon.
```
...
```
Create a Heat stack. Replace $NET_ID and $SUBNET_ID with the Neutron network and subnet IDs used to spawn tenant instances. Replace $ADMIN_KEY with the name of your Nova keypair. Replace $IMAGE_NAME with the name of the Heat image used to spawn Nova instances.

**Note:** The example template used below requires an existing Neutron network/subnet, Heat image and Nova keypair. Create these if they do not exist in your deployment. Use the official [Openstack documenttion](http://www.docs.openstack.org) for further assistance.
